...
class LocalStream:
    ...
    def _backend_connected(self) -> bool:
        ...
        try:
            ...
            handler_state = {}
        return any(handler_state.get(attr) is not None for attr in ("connection", "session", "conversation"))
    ...
    def _persist_elevenlabs_api_key(self, key: str) -> None:
        ...
        self._persist_env_value(ELEVENLABS_API_KEY_ENV, key)
    ...
    def _init_settings_ui_if_needed(self) -> None:
        ...
        def _status_payload() -> dict[str, object]:
            ...
            has_elevenlabs_key = self._has_required_key(ELEVENLABS_BACKEND)
            ...
            can_proceed_with_elevenlabs = has_elevenlabs_key
            ...
            return {
                ...
                "can_proceed_with_elevenlabs": can_proceed_with_elevenlabs,
        ...
        def _set_backend(payload: BackendPayload) -> JSONResponse:
            ...
            if backend not in {OPENAI_BACKEND, GEMINI_BACKEND, HF_BACKEND, ELEVENLABS_BACKEND}:
            ...
            if backend == ELEVENLABS_BACKEND and not api_key and not self._has_required_key(ELEVENLABS_BACKEND):
                return JSONResponse({"ok": False, "error": "empty_key"}, status_code=400)
            ...
            if backend == ELEVENLABS_BACKEND and api_key:
                self._persist_elevenlabs_api_key(api_key)
            if backend == HF_BACKEND:
