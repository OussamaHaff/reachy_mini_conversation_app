...
AutoModelForImageTextToText: Any | None = None
...
def _load_vision_dependencies(
    *,
    ...
    """Import optional local-vision dependencies on demand."""
    global torch, Image, AutoProcessor, AutoModelForImageTextToText, snapshot_download
    ...
    if include_image and Image is None:
        ...
        Image = image_module
    if include_processor and AutoProcessor is None:
        from transformers import AutoProcessor as auto_processor

        AutoProcessor = auto_processor
    if include_model and AutoModelForImageTextToText is None:
        from transformers import AutoModelForImageTextToText as auto_model_for_image_text_to_text

        AutoModelForImageTextToText = auto_model_for_image_text_to_text
    if include_downloader and snapshot_download is None:
...
class VisionProcessor:
    ...
    def _determine_device(self) -> str:
        ...
        torch_module, _, _, _, _ = _load_vision_dependencies(
            include_image=False,
    ...
    def initialize(self) -> None:
        ...
        torch_module, _, auto_processor, _, _ = _load_vision_dependencies(
            include_image=False,
            include_model=False,
            include_downloader=False,
        )
        ...
        processor = cast(
            _VisionProcessor,
            auto_processor.from_pretrained(self.vision_config.model_path),  # type: ignore[no-untyped-call]
        )

        model_kwargs: dict[str, object] = {
            "dtype": torch_module.bfloat16 if self.device == "cuda" else torch_module.float32,
        }
        _, _, _, auto_model_for_image_text_to_text, _ = _load_vision_dependencies(
            include_image=False,
            include_processor=False,
            include_downloader=False,
        )
        ...
        model = cast(
            _VisionModel,
            auto_model_for_image_text_to_text.from_pretrained(
    ...
    def process_image(
        ...
        torch_module, image_module, _, _, _ = _load_vision_dependencies(
            include_processor=False,
            include_model=False,
            include_downloader=False,
        )
        ...
        model = self.model
        rgb_image = image_module.fromarray(np.ascontiguousarray(frame[..., ::-1]))
...
def initialize_vision_processor() -> VisionProcessor:
    ...
    try:
        _, _, _, _, snapshot_download_func = _load_vision_dependencies(
            include_image=False,
            include_processor=False,
            include_model=False,
        )
        ...
        snapshot_download_func(repo_id=model_id, repo_type="model", cache_dir=cache_dir)
