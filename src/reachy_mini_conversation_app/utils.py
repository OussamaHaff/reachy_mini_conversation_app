...
def parse_args() -> tuple[argparse.Namespace, list]:  # type: ignore
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="[Optional] Robot hostname or IP address. When provided, forces network connection mode.",
    )
...
def log_connection_troubleshooting(
    ...
    if host is not None:
        logger.error("  2. Verify the robot is reachable: ping %s", host)
    else:
        logger.error("  2. For wireless connection, add --host <hostname-or-ip>")

    if robot_name is not None:
        logger.error("  3. Daemon must be started with: --robot-name '%s'", robot_name)
    else:
        logger.error("  3. If daemon uses --robot-name, add the same flag here: --robot-name <name>")
