...
- Real-time audio conversation loop with `fastrtc` for low-latency streaming. Supported backends:
  ...
  - **ElevenLabs Conversational AI** (`gpt-4o-mini` agent LLM) - requires `ELEVENLABS_API_KEY`.
...
| `ELEVENLABS_API_KEY` | Required for ElevenLabs Conversational AI mode. |
...
To use ElevenLabs Conversational AI:
...
| `--host` | `None` | Optional. Connect to a robot hostname or IP address and force network connection mode. |
